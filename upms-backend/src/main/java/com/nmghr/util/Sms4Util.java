...
public class Sms4Util {
  ...
  private static byte[] key = "JeF8U9wHFOMfs2S3".getBytes();
  ...
  public static byte[] encodeSMS4(String plaintext, byte[] key)
    ...
    return Sms4Util.encodeSMS4(encryptDES(plaintext).getBytes(), key);
    /*
  ...
  public static String Encryption(String encodingStr) {
    return getEncryptString(encodingStr, key);
  }

  public static String Encryption(String encodingStr, String key) {
    return getEncryptString(encodingStr, key.getBytes());
  }

  private static String getEncryptString(String encodingStr, byte[] key) {
    try {
      byte[] enOut = Sms4Util.encodeSMS4(encodingStr, key);
  ...
  public static String Decrypt(String Redaction) {
    return getDecryptString(Redaction, key);
  }

  public static String Decrypt(String Redaction, String key) {
    return getDecryptString(Redaction, key.getBytes());
  }

  private static String getDecryptString(String Redaction, byte[] key) {
    try {
      byte[] bytes = Convert.hexStringToBytes(Redaction);
      String deOutStr = Sms4Util.decodeSMS4toString(bytes, key);
